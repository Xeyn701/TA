...
import RealtimeChart from '../../charts/RealtimeChart3';
import { tailwindConfig, hexToRGB } from '../../utils/Utils';
import axios from 'axios';

function KelembapanMonitor() {
  const [data, setData] = useState([]);
  const [labels, setLabels] = useState([]);

  const fetchKelembapanData = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/kelembapan`);
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
      <header className="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center">
        <h2 className="font-semibold text-gray-800 dark:text-gray-100">Kelembapan Udara</h2>
      </header>
...
export default KelembapanMonitor;